...
description: Agent skills for working with Minions Oss-releases MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-oss-releases data.
---

# Minions Oss-releases Agent Skills

Skills for agents operating on the `minions-oss-releases` toolbox.
...
```bash
# TypeScript
pnpm add @minions-oss-releases/sdk
...
The `oss-releases` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-oss-releases/sdk/schemas';
...
```python
from minions_oss_releases.schemas import custom_types
...
- This agent only writes to `minions-oss-releases` — it reads from other toolboxes but never writes to them
